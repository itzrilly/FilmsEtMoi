...
import EnlargeShrink from '../Animations/EnlargeScreen'
...
class FilmDetail extends React.Component {
  ...
  _toggleWatch() {
    const action = { type: 'TOGGLE_WATCH', value: this.state.film }
    this.props.dispatch(action)
  }

  _displayTextBtn() {
    var defaultText = 'Marquer comme vu'
    if(this.props.watchedFilm.findIndex(item => item.id === this.state.film.id) !== -1){
      defaultText = 'Marquer comme non vu'
    }
    return defaultText
  }

  componentDidUpdate() {
    console.log(this.props.watchedFilm)
  }
    ...
    if (film != undefined) {
      return (
        <View style={{ flex: 1 }}>
          ...
          <Button style={{ flex: 1 }}
            title={this._displayTextBtn()}
            onPress={() => this._toggleWatch()}
          />
        </View>
      )
    }
